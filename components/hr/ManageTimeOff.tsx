...
import { TimeOffRequest, RequestStatus, User } from '../../types';
            ...
            <tbody className="bg-white divide-y divide-slate-200">
              {filteredRequests.map(req => (
                <tr key={req.id}>
                  ...
                  <td className="px-6 py-4 whitespace-normal text-sm text-slate-500 max-w-xs">
                    {req.justification && <p className="truncate" title={req.justification}>{req.justification}</p>}
                    {req.approvedByLeader && (
                        <p className="mt-1 text-xs italic text-slate-600">
                            Liberado por: {req.approvedByLeader}
                        </p>
                    )}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
